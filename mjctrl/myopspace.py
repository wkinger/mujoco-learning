...
def main() -> None:
    ...
    with mujoco.viewer.launch_passive(
        ...
        while viewer.is_running():
            ...
            # 使用分解求解线性系统 Mx=y：x=(LTDL)−1*y。通过这种方式求M_inv效率更高
            # model, data提供惯性矩阵M， M_inv是x， np.eye(model.nv)是y model.nv是自由度数量。
            mujoco.mj_solveM(model, data, M_inv, np.eye(model.nv))
            # 任务空间惯性矩阵的逆 Mx_inv = J*M_inv*J^T
            Mx_inv = jac @ M_inv @ jac.T
            # 求任务空间惯性矩阵 Mx = (Mx_inv)^-1（判断行列式避免奇异）
            ...
            actual_pose = [data.site(site_id).xpos[0], data.site(site_id).xpos[1], data.site(site_id).xpos[2]
                           ,0,0,0]
            mujoco.mju_quat2Vel(actual_pose[3:], site_quat, 1.0)
            target_pose = [data.mocap_pos[mocap_id][0], data.mocap_pos[mocap_id][1], data.mocap_pos[mocap_id][2]
                           ,0,0,0]
            mujoco.mju_quat2Vel(target_pose[3:], data.mocap_quat[mocap_id], 1.0)
            plotter.add_data(data.time, actual_pose, target_pose)
            viewer.sync()
