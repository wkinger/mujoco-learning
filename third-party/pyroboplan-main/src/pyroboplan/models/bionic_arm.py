...
def load_models(use_sphere_collisions=False):
    ...
    urdf_filepath = "/home/kuanwang/workspace/model_files/single_arm/bionic_arm.urdf"
...
def add_object_collisions(model, collision_model, visual_model, inflation_radius=0.0):
    ...
    collision_names = [
        cobj.name for cobj in collision_model.geometryObjects if "Right" in cobj.name
    ]
    obstacle_names = [
        "ground_plane",
        "obstacle_box_1",
    ...
    set_collisions(model, collision_model, "Torso_Link", "ground_plane", False)

def get_collision_pair_indices_from_bodies(model, collision_model, body_list):
    return get_collision_pair_indices_from_bodies(model, collision_model, body_list)
